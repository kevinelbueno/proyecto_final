...
def add_art(request):
    if request.method == 'GET':
        ...
        if form.is_valid():
            ...
            if request.user.is_superuser:  # Si el usuario es administrador
                return redirect('art_list_AD')
            else:
                return redirect('art_list')  # Si el usuario no es administrador
...
def art_list_AD(request):
    arts = Art.objects.all()
    return render(request, 'art_list_Admin.html', {'arts': arts})
...
def delete_art(request, art_id):
    ...
    return render(request, 'art_detail_Admin.html')
